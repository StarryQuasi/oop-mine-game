...
namespace gui
{
	...
	Container* Container::setSize(olc::vi2d v)
	{
		...
		if (size != v && debug)
			std::println("{} size set to {},{}", getDebugName().empty() ? typeid(*this).name() : getDebugName(), v.x, v.y);
		if (size != v)
			invalidate();
		size = v;
		return this;
	...
	{
		if (pos != v && debug)
			std::println("{} pos set to {},{}", getDebugName().empty() ? typeid(*this).name() : getDebugName(), v.x, v.y);
		if (pos != v)
			invalidate();
		pos = v;
		return this;
	...
	{
		if (visible != v && debug)
			std::println("{} visible set to {}", getDebugName().empty() ? typeid(*this).name() : getDebugName(), v);
		visible = v;
		return this;
	}
	...
	void Container::setAbsolutePos(olc::vi2d v)
	{
		...
		if (absPos != v && debug)
			std::println("{} absPos set to {},{}", getDebugName().empty() ? typeid(*this).name() : getDebugName(), v.x, v.y);
		absPos = v;
	}

	void Container::removeChild(int id)
	{
		auto it = std::ranges::find(children, id, [id](const std::unique_ptr<Container>& child)
			{
				return child->getId();
			});
	...
	void Container::setDebug(bool v)
	{
		debug = v;
	}
	...
	{
		MouseEvent local = e;
		local.pos = parent ? parent->screenToLocal(local.globalPos) : local.globalPos;
		if (!isPointInside(local.pos))
			return false;
		if (debug)
			std::println("mouse down {}({},{},{},{})", getDebugName().empty() ? typeid(*this).name() : getDebugName(),
				getAbsolutePos().x, getAbsolutePos().y, getSize().x, getSize().y);
		for (auto& child : children | std::views::reverse)
	...
	void Container::onMouseDown(std::function<bool(Container&, const MouseEvent&)> handler) { mouseDownHandler = std::move(handler); }

	void Container::onMouseDrag(std::function<bool(Container&, const MouseDragEvent&)> handler) { mouseDragHandler = std::move(handler); }

	void Container::onMouseUp(std::function<bool(Container&, const MouseEvent&)> handler) { mouseUpHandler = std::move(handler); }
	...
	void Container::updateLayout()
	{
		...
		// 2nd pass: same as above, but nothing is invalidated, now the validation loop ends
	...
	void Container::draw(OopMineGame& game) const
	{
		...
		if (!assetName.empty())
		{
			...
			game.DrawDecal(getAbsolutePos(), decal, (olc::vf2d)getDrawSize() / assetSize);
		}
		...
		if (debug)
		{
			if (margin != olc::vi2d())
			{
				...
				game.FillRectDecal(pos - m + olc::vi2d{ 0, m.y + s.y }, olc::vi2d{ s.x + m.x * 2, m.y }, pixel);
				game.FillRectDecal(pos - m + olc::vi2d{ 0, m.y }, olc::vi2d{ m.x, s.y }, pixel);
				game.FillRectDecal(pos - m + olc::vi2d{ m.x + s.x, m.y }, olc::vi2d{ m.x, s.y }, pixel);
			}
			if (padding != olc::vi2d())
			{
				...
				game.FillRectDecal(pos + olc::vi2d{ 0, s.y - p.y }, olc::vi2d{ s.x, p.y }, pixel);
				game.FillRectDecal(pos + olc::vi2d{ 0, p.y }, olc::vi2d{ p.x, s.y - p.y * 2 }, pixel);
				game.FillRectDecal(pos + olc::vi2d{ s.x - p.x, p.y }, olc::vi2d{ p.x, s.y - p.y * 2 }, pixel);
			}
			game.DrawRectDecal(getAbsolutePos(), getDrawSize(), olc::Pixel(0xC0FFFFFF));
		}
	}
}
