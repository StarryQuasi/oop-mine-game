...
namespace gui
{
	...
	bool FlowContainer::getAutoSizing() const
	{
		return autoSizing;
	}
	...
	void FlowContainer::updateLayout()
	{
		...
		if (autoSizing)
		{
			...
			{
				main += (child->getDrawSize() + child->getMargin() * 2) * direction.getVector();
				secondary = secondary.max((child->getDrawSize() + child->getMargin() * 2) * direction.getSecondaryVector());
			}
			...
			{
				child->setPos(pos);
				pos += (child->getDrawSize() + child->getMargin() * 2) * direction.getVector();
			}
		}
