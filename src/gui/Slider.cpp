...
float Slider::getMin() const { return min; }

float Slider::getMax() const { return max; }
...
void Slider::updateLayout()
{
	...
	const int regionWidth =
		handleContainer->getSize().x - handleEle->getSize().x;
	const int handleX =
		(int)((value - min) / std::max(max - min, 0.000001f) * regionWidth);
